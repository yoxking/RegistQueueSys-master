...
{
  public class OperateFlowsDAL: IOperateFlows
  {
        #region sql
        private const string SQL_GET_ALL_RECORDS = @"Select *  From OperateFlows Where AppCode like @AppCode And ValidityState=1";
        private const string SQL_GET_RECORDS_BY_NO = @"Select * From OperateFlows Where   AppCode like @AppCode And   ValidityState=1 And OFlowNo=@OFlowNo";
        private const string SQL_GET_NAME_BY_NO = @"Select Name From OperateFlows Where   AppCode like @AppCode And   ValidityState=1 And OFlowNo=@OFlowNo";
        private const string SQL_ADD_RECORD = @"Insert into OperateFlows
                                              (OFlowNo,DataFlag,OperatType,DataFrom,RUserNo,StafferNo,RegistDate,OperatTime,RoomNo,RoomName,OptIndex,OptName,OptDesc,Opter1No,Opter1Name,Opter2No,Opter2Name,Opter3No,Opter3Name,Opter4No,Opter4Name,Opter5No,Opter5Name,OperatState,BranchNo,AddOptor,AddDate,ModOptor,ModDate,ValidityState,Comments,AppCode)
                                              values(@OFlowNo,@DataFlag,@OperatType,@DataFrom,@RUserNo,@StafferNo,@RegistDate,@OperatTime,@RoomNo,@RoomName,@OptIndex,@OptName,@OptDesc,@Opter1No,@Opter1Name,@Opter2No,@Opter2Name,@Opter3No,@Opter3Name,@Opter4No,@Opter4Name,@Opter5No,@Opter5Name,@OperatState,@BranchNo,@AddOptor,@AddDate,@ModOptor,@ModDate,@ValidityState,@Comments,@AppCode)";
        private const string SQL_UPDATE_RECORD = @"Update OperateFlows set
                                                 OFlowNo=@OFlowNo,DataFlag=@DataFlag,OperatType=@OperatType,DataFrom=@DataFrom,RUserNo=@RUserNo,StafferNo=@StafferNo,RegistDate=@RegistDate,OperatTime=@OperatTime,RoomNo=@RoomNo,RoomName=@RoomName,OptIndex=@OptIndex,OptName=@OptName,OptDesc=@OptDesc,Opter1No=@Opter1No,Opter1Name=@Opter1Name,Opter2No=@Opter2No,Opter2Name=@Opter2Name,Opter3No=@Opter3No,Opter3Name=@Opter3Name,Opter4No=@Opter4No,Opter4Name=@Opter4Name,Opter5No=@Opter5No,Opter5Name=@Opter5Name,OperatState=@OperatState,BranchNo=@BranchNo,AddOptor=@AddOptor,AddDate=@AddDate,ModOptor=@ModOptor,ModDate=@ModDate,ValidityState=@ValidityState,Comments=@Comments,AppCode=@AppCode 
                                                 Where  AppCode like @AppCode And   ValidityState=1 And OFlowNo=@OFlowNo  And Version=@Version";
        private const string SQL_HARD_DELETE_RECORD = @"Delete From OperateFlows Where   AppCode like @AppCode And   OFlowNo=@OFlowNo ";
        private const string SQL_SOFT_DELETE_RECORD = @"Update OperateFlows set ValidityState=0 Where   AppCode like @AppCode And   ValidityState=1 And OFlowNo=@OFlowNo";
        private const string SQL_HARD_DELETE_BY_CONDTION = @"Delete From OperateFlows Where   AppCode like @AppCode ";
        private const string SQL_SOFT_DELETE_BY_CONDTION = @"Update OperateFlows set ValidityState=0 Where   AppCode like @AppCode And   ValidityState=1 ";
        private const string SQL_GET_RECORDS_BY_CLASSNO = @"Select * From OperateFlows Where    AppCode like @AppCode And   ValidityState=1 And StafferNo=@StafferNo";
        private const string SQL_GET_COUNT_BY_CONDITION = @"Select Count(*) From OperateFlows Where   AppCode like @AppCode  And   ValidityState=1 ";
        #endregion
        ...
        public OperateFlowsDAL(string sConnStr,string sAppCode)
        {
        ...
        public OperateFlowsCollections GetAllRecords()
        {
            ...
            OperateFlowsCollections infos = null;
            OperateFlows info = null;
            ...
            try
            {
                ...
                if (reader.HasRows)
                {
                    infos = new OperateFlowsCollections();
                    while (reader.Read())
                    {
                        info = new OperateFlows();
                        // 设置对象属性
        ...
        public OperateFlowsCollections GetRecordsByNo(string sNo)
        {
            SqlConnection connection = null;
            SqlDataReader reader = null;
            OperateFlowsCollections infos = null;
        ...
        public int AddNewRecord(OperateFlows info)
        {
            SqlConnection connection = null;
            ...
            try
            {
                ...
                paras[7].Value = info.dOperatTime;
                paras[8].Value = info.sRoomNo;
                paras[9].Value = info.sRoomName;
                paras[10].Value = info.iOptIndex;
                paras[11].Value = info.sOptName;
                paras[12].Value = info.sOptDesc;
                paras[13].Value = info.sOpter1No;
                paras[14].Value = info.sOpter1Name;
                paras[15].Value = info.sOpter2No;
                paras[16].Value = info.sOpter2Name;
                paras[17].Value = info.sOpter3No;
                paras[18].Value = info.sOpter3Name;
                paras[19].Value = info.sOpter4No;
                paras[20].Value = info.sOpter4Name;
                paras[21].Value = info.sOpter5No;
                paras[22].Value = info.sOpter5Name;
                paras[23].Value = info.iOperatState;
                paras[24].Value = info.sBranchNo;
                paras[25].Value = info.sAddOptor;
                paras[26].Value = info.dAddDate;
                paras[27].Value = info.sModOptor;
                paras[28].Value = info.dModDate;
                paras[29].Value = info.iValidityState;
                paras[30].Value = info.sComments;
                paras[31].Value = info.sAppCode;
        ...
        public int UpdateRecord(OperateFlows info)
        {
            SqlConnection connection = null;
            ...
            try
            {
                ...
                paras[30].Value = info.sComments;
                paras[31].Value = info.sAppCode;
                paras[32].Value = StringHelper.ConvertToBytes(info.sVersion);
  ...
  catch (Exception ex)
 {
  ...
  finally
  {
     if (connection != null)
        ...
        public OperateFlowsCollections GetRecords_Paging(SqlModel s_model)
        {
            SqlConnection connection = null;
            SqlDataReader reader = null;
            OperateFlowsCollections infos = null;
       ...
       /// <param name=" obj_info">主题对象</param>
        ...
        internal static void PutObjectProperty(OperateFlows obj_info, SqlDataReader reader)
        {
