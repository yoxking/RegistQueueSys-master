...
{
    public class StafferRotaBusiness
    {
        private volatile static StafferRotaBusiness _instance = null;
        private static readonly object lockHelper = new object();
        public static StafferRotaBusiness CreateInstance()
        {
            if (_instance == null)
            {
                lock (lockHelper)
                {
                    if (_instance == null)
                        _instance = new StafferRotaBusiness();
                }
            }
            return _instance;
        }
        public string GetAllRecords()
        {
            try
            {
                ...
                StafferRotaBLL infoBoss = new StafferRotaBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                StafferRotaCollections infoColl = infoBoss.GetRecordsByPaging(ref count, 1, 100, sWhere);
                if (infoColl != null)
                {
        ...
        public string GetRecordByNo(string sNo)
        {
            try
            {
                string sResult = "";
                StafferRotaBLL infoBoss = new StafferRotaBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                StafferRota info = infoBoss.GetRecordByNo(sNo);
                if (info != null)
                {
        ...
        public string GetRecordsByPaging(string pageIndex, string pageSize, string condition)
        {
            try
            {
                ...
                StafferRotaBLL infoBoss = new StafferRotaBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                StafferRotaCollections infoColl = infoBoss.GetRecordsByPaging(ref count, int.Parse(pageIndex), int.Parse(pageSize), sWhere);
                if (infoColl != null)
                {
                    sResult = JsonConvert.SerializeObject(infoColl, Formatting.None);
                }
        ...
        public string PostRecord(string sInfo)
        {
            try
            {
                ...
                StafferRota info1 = JsonConvert.DeserializeObject<StafferRota>(sInfo);
                if (info1 != null)
                {
                    StafferRotaBLL infoBoss = new StafferRotaBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                    StafferRota info2 = infoBoss.GetRecordByNo(info1.sRotaNo);
                    if (info2 == null)
                    {
                        info2 = info1;
