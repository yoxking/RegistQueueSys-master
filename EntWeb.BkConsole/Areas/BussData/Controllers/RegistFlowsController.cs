...
{
    public class RegistFlowsController : frmMainController
    {
        ...
        public override ActionResult List()
        {
            try
            {
                ...
                RegistFlowsCollections infoColl = infoBLL.GetRecordsByPaging(ref PageCount, PageIndex, this.PageSize, Condition);
        ...
        //[(Message = "病人预约信息编辑(Edit)")]
        public override ActionResult Edit(string id)
        {
            try
            {
                RegistFlowsBLL infoBLL = new RegistFlowsBLL(PublicHelper.Get_ConnStr(), PublicHelper.Get_AppCode());
                RegistFlows info = infoBLL.GetRecordByNo(id);
        ...
        public override ActionResult Save()
        {
            ...
            try
            {
                ...
                string RFlowNo = Request.Form["RFlowNo"].ToString();
                string RUserNo = Request.Form["RUserNo"].ToString();
                string ServiceNo = Request.Form["ServiceNo"].ToString();
                string DataFrom = Request.Form["DataFrom"].ToString();
                string StafferNo = Request.Form["StafferNo"].ToString();
                DateTime StartDate = DateTime.Parse(Request.Form["StartDate"].ToString());
                DateTime EnditDate = DateTime.Parse(Request.Form["EnditDate"].ToString());
                string Comments = Request.Form["Comments"].ToString();

                RegistFlowsBLL infoBLL = new RegistFlowsBLL(PublicHelper.Get_ConnStr(), PublicHelper.Get_AppCode());
                RegistFlows info = infoBLL.GetRecordByNo(RFlowNo);
                ...
                if (info == null)
                {
                    ...
                    info.sRFlowNo = RFlowNo;
                    info.iDataFlag = 0;
                    info.sRUserNo = RUserNo;
                    info.sServiceNo = ServiceNo;
                    info.iRegistType = 1;
                    info.sDataFrom = DataFrom;
                    info.dRegistDate = DateTime.Now;
                    info.sStafferNo = StafferNo;
                    info.dStartDate = StartDate;
                    info.dEnditDate = EnditDate;
                    info.iRegistState = 0;
