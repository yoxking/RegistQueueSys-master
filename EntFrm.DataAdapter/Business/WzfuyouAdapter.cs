...
namespace EntFrm.DataAdapter.Business
{
    ...
    /// 重庆万州妇幼保健院
    /// </summary>
    public class WzfuyouAdapter: IAdapterBusiness
    {
        ...
        public bool updatePatientList()
        {
            try
            {
                ...
                string condition = "  regist_type=0 and register_time> to_date('" + DateTime.Now.AddMinutes(-30).ToString("yyyy-MM-dd HH:mm:ss") + "','yyyy-mm-dd hh24:mi:ss') ";
        ...
        public bool updateRegisteList()
        {
            try
            {
                ...
                string condition = " regist_type=1 and  register_time> to_date('" + DateTime.Now.ToString("yyyy-MM-dd 00:00:00") + "','yyyy-mm-dd hh24:mi:ss')  ";
                //string condition = " 登记时间> to_date('" + DateTime.Now.ToString("yyyy-MM-dd 00:00:00") + "','yyyy-mm-dd hh24:mi:ss') ";
        ...
        private string getBranchNoByBranchId(string branchId)
        {
            ...
            if ("421,420,424".Contains(branchId))
            {
                result = "420";
            }
            else if ("56,57,423,426".Contains(branchId))
            {
                result = "423";
            }
            else
        ...
        private string getServiceNoByServiceName(string serviceName)
        {
            try
            {
                ...
                ServiceInfoCollections infoColl = infoBoss.GetRecordsByPaging(ref count, 1, 1, "ServiceName = '超声科'");
