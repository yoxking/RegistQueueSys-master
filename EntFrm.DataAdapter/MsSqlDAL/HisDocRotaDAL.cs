    ...
    public class HisDocRotaDAL
    {
        ...
        public List<HisDoctorRota> GetRecordsByClassNo1(string sClassNo)
        {
            ...
            try
            {
                ...
                {
                    new SqlParameter(PARAM_COUNTERID,SqlDbType.NVarChar,50)
                };
                paras[0].Value = sClassNo;

                connection = SqlHelper.GetConnection(connectionStr);
                reader = SqlHelper.ExecuteReader(connection, CommandType.Text, SQL_GET_RECORDS_BY_CLASSNO1, paras);
        ...
        public List<HisDoctorRota> GetRecordsByClassNo2(string sClassNo)
        {
            ...
            try
            {
                SqlParameter[] paras = new SqlParameter[]
                {
                    new SqlParameter(PARAM_DOCTORID,SqlDbType.NVarChar,50)
                };
                paras[0].Value = sClassNo;

                connection = SqlHelper.GetConnection(connectionStr);
                reader = SqlHelper.ExecuteReader(connection, CommandType.Text, SQL_GET_RECORDS_BY_CLASSNO2, paras);
        ...
        public HisDoctorRota GetRecordByNo(string sNo)
        {
            ...
            try
            {
                SqlParameter[] paras = new SqlParameter[]
                {
                    new SqlParameter(PARAM_ID,SqlDbType.NVarChar,50)
                };
                paras[0].Value = sNo;
        ...
        internal static void PutObjectProperty(HisDoctorRota obj_info, SqlDataReader reader)
        {
