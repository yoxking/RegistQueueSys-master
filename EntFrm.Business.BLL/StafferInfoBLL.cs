...
{
    public class StafferInfoBLL
    {

        private string connStr;
        private string appCode;

        public  StafferInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IStafferInfo infoDAL = StafferInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" StafferNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(StafferInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sStafferNo))
            {
                IStafferInfo infoDAL = StafferInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(StafferInfo info)
        {
            try
            {
                     IStafferInfo  infoDAL = StafferInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  StafferInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IStafferInfo infoDAL = StafferInfoFactory.Create(this.connStr, this.appCode);
                StafferInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public StafferInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IStafferInfo infoDAL = StafferInfoFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "StafferInfo";
        ...
        public StafferInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
