...
{
    public class OperateHistoryBLL
    {

        private string connStr;
        private string appCode;

        public  OperateHistoryBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IOperateHistory infoDAL = OperateHistoryFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" HFlowNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(OperateHistory info)
        {
            try
            {
            if (!ValidateRepeat(info.sHFlowNo))
            {
                IOperateHistory infoDAL = OperateHistoryFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(OperateHistory info)
        {
            try
            {
                     IOperateHistory  infoDAL = OperateHistoryFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  OperateHistory GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IOperateHistory infoDAL = OperateHistoryFactory.Create(this.connStr, this.appCode);
                OperateHistoryCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public OperateHistoryCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IOperateHistory infoDAL = OperateHistoryFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "OperateHistory";
        ...
        public OperateHistoryCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
