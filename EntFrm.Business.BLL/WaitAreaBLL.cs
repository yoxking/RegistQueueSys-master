...
{
    public class WaitAreaBLL
    {

        private string connStr;
        private string appCode;

        public  WaitAreaBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IWaitArea infoDAL = WaitAreaFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" WAreaNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(WaitArea info)
        {
            try
            {
            if (!ValidateRepeat(info.sWAreaNo))
            {
                IWaitArea infoDAL = WaitAreaFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(WaitArea info)
        {
            try
            {
                     IWaitArea  infoDAL = WaitAreaFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  WaitArea GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IWaitArea infoDAL = WaitAreaFactory.Create(this.connStr, this.appCode);
                WaitAreaCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public WaitAreaCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IWaitArea infoDAL = WaitAreaFactory.Create(this.connStr, this.appCode);
        ...
        public WaitAreaCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
