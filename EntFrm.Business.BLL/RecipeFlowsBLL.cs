...
{
    public class RecipeFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  RecipeFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IRecipeFlows infoDAL = RecipeFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(RecipeFlows info)
        {
            try
            {
        ...
        public  bool UpdateRecord(RecipeFlows info)
        {
            try
            {
                     IRecipeFlows  infoDAL = RecipeFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  RecipeFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IRecipeFlows infoDAL = RecipeFlowsFactory.Create(this.connStr, this.appCode);
                RecipeFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public RecipeFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IRecipeFlows infoDAL = RecipeFlowsFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "RecipeFlows";
        ...
        public RecipeFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
