...
{
    public class StafferRotaBLL
    {

        private string connStr;
        private string appCode;

        public  StafferRotaBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IStafferRota infoDAL = StafferRotaFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(StafferRota info)
        {
            try
            {
        ...
        public  bool UpdateRecord(StafferRota info)
        {
            try
            {
                     IStafferRota  infoDAL = StafferRotaFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  StafferRota GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IStafferRota infoDAL = StafferRotaFactory.Create(this.connStr, this.appCode);
                StafferRotaCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public StafferRotaCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IStafferRota infoDAL = StafferRotaFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "StafferRota";
        ...
        public StafferRotaCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
