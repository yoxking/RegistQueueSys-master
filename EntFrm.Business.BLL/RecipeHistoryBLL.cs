...
{
    public class RecipeHistoryBLL
    {

        private string connStr;
        private string appCode;

        public  RecipeHistoryBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IRecipeHistory infoDAL = RecipeHistoryFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(RecipeHistory info)
        {
            try
            {
        ...
        public  bool UpdateRecord(RecipeHistory info)
        {
            try
            {
                     IRecipeHistory  infoDAL = RecipeHistoryFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  RecipeHistory GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IRecipeHistory infoDAL = RecipeHistoryFactory.Create(this.connStr, this.appCode);
                RecipeHistoryCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public RecipeHistoryCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IRecipeHistory infoDAL = RecipeHistoryFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "RecipeHistory";
        ...
        public RecipeHistoryCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
