...
{
    public class ViewRecipeFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  ViewRecipeFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IViewRecipeFlows infoDAL = ViewRecipeFlowsFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" RFlowNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(ViewRecipeFlows info)
        {
            try
            {
            if (!ValidateRepeat(info.sRFlowNo))
            {
                IViewRecipeFlows infoDAL = ViewRecipeFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(ViewRecipeFlows info)
        {
            try
            {
                     IViewRecipeFlows  infoDAL = ViewRecipeFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  ViewRecipeFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IViewRecipeFlows infoDAL = ViewRecipeFlowsFactory.Create(this.connStr, this.appCode);
                ViewRecipeFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public ViewRecipeFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IViewRecipeFlows infoDAL = ViewRecipeFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public ViewRecipeFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
