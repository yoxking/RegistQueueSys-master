...
{
    public class ViewServiceRotaBLL
    {

        private string connStr;
        private string appCode;

        public  ViewServiceRotaBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IViewServiceRota infoDAL = ViewServiceRotaFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" RotaNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(ViewServiceRota info)
        {
            try
            {
            if (!ValidateRepeat(info.sRotaNo))
            {
                IViewServiceRota infoDAL = ViewServiceRotaFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(ViewServiceRota info)
        {
            try
            {
                     IViewServiceRota  infoDAL = ViewServiceRotaFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  ViewServiceRota GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IViewServiceRota infoDAL = ViewServiceRotaFactory.Create(this.connStr, this.appCode);
                ViewServiceRotaCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public ViewServiceRotaCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IViewServiceRota infoDAL = ViewServiceRotaFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "ViewServiceRota";
        ...
        public ViewServiceRotaCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
