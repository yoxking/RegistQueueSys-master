...
{
    public class ViewStafferRotaBLL
    {

        private string connStr;
        private string appCode;

        public  ViewStafferRotaBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IViewStafferRota infoDAL = ViewStafferRotaFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(ViewStafferRota info)
        {
            try
            {
        ...
        public  bool UpdateRecord(ViewStafferRota info)
        {
            try
            {
                     IViewStafferRota  infoDAL = ViewStafferRotaFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  ViewStafferRota GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IViewStafferRota infoDAL = ViewStafferRotaFactory.Create(this.connStr, this.appCode);
                ViewStafferRotaCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public ViewStafferRotaCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IViewStafferRota infoDAL = ViewStafferRotaFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "ViewStafferRota";
        ...
        public ViewStafferRotaCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
