...
{
    public class RecipeDetailsBLL
    {

        private string connStr;
        private string appCode;

        public  RecipeDetailsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IRecipeDetails infoDAL = RecipeDetailsFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" RecipeNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(RecipeDetails info)
        {
            try
            {
            if (!ValidateRepeat(info.sRecipeNo))
            {
                IRecipeDetails infoDAL = RecipeDetailsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(RecipeDetails info)
        {
            try
            {
                     IRecipeDetails  infoDAL = RecipeDetailsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  RecipeDetails GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IRecipeDetails infoDAL = RecipeDetailsFactory.Create(this.connStr, this.appCode);
                RecipeDetailsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public RecipeDetailsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IRecipeDetails infoDAL = RecipeDetailsFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "RecipeDetails";
        ...
        public RecipeDetailsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
