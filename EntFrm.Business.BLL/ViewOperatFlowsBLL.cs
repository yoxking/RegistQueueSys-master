...
{
    public class ViewOperatFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  ViewOperatFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IViewOperatFlows infoDAL = ViewOperatFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(ViewOperatFlows info)
        {
            try
            {
        ...
        public  bool UpdateRecord(ViewOperatFlows info)
        {
            try
            {
                     IViewOperatFlows  infoDAL = ViewOperatFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  ViewOperatFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IViewOperatFlows infoDAL = ViewOperatFlowsFactory.Create(this.connStr, this.appCode);
                ViewOperatFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public ViewOperatFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IViewOperatFlows infoDAL = ViewOperatFlowsFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "ViewOperatFlows";
        ...
        public ViewOperatFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
