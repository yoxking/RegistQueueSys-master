...
namespace EntFrm.Business.BLL
{
    public class MessageInfoBLL
    {
        ...
        public  MessageInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IMessageInfo infoDAL = MessageInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" MessNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(MessageInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sMessNo))
            {
                IMessageInfo infoDAL = MessageInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(MessageInfo info)
        {
            try
            {
                     IMessageInfo  infoDAL = MessageInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  MessageInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IMessageInfo infoDAL = MessageInfoFactory.Create(this.connStr, this.appCode);
                MessageInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public MessageInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IMessageInfo infoDAL = MessageInfoFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "MessageInfo";
        ...
        public MessageInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
